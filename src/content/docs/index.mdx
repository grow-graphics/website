---
title: Welcome to Learn Graphics GD
description: GDExtension for the Go programming Language
template: splash
hero:
  tagline: Community Supported Godot GDExtension bindings for the Go programming Language!
  image:
    file: ../../assets/gdgrowlogo.svg
  actions:
    - text: Documentation
      link: /documentation
      icon: right-arrow
      variant: primary
    - text: GitHub Repository
      link: https://github.com/grow-graphics/gd
      icon: external
---

import { Card, CardGrid } from '@astrojs/starlight/components';

## Next steps

<CardGrid stagger>
	<Card title="Graphics GD Documentation" icon="open-book">
		![alt text](../../assets/gdgrowlogo.svg)
		*Go Community Supported GDExtension bindings for Godot*

		- [Our Documentation](https://learn.grow.graphics/documentation)
	</Card>
	<Card title="Github Repository" icon="open-book">
		- [Grow-Graphics GD GitHub Repo](https://github.com/grow-graphics/gd)
		  [![Star History Chart](https://api.star-history.com/svg?repos=grow-graphics/gd&type=Date)](https://star-history.com/#grow-graphics/gd&Date)
	</Card>
	<Card title="Godot Game Engine" icon="add-document">
		![Godot Game Engine](../../assets/godot-logo.svg)
		- [Godot Game Engine](https://godotengine.org)
		- [Godot's Documentation](https://godotengine.org)
		- [GDExtension](https://docs.godotengine.org/en/stable/tutorials/scripting/gdextension/index.html)
	</Card>
	<Card title="Go Programming Language" icon="open-book">
		![Go Programming Language](../../assets/go-gopher.jpg)
		- [Go Programming Language](https://go.dev/)
		- [Effective Go](https://go.dev/doc/effective_go)
		- [Go Standard Library](https://pkg.go.dev/std)
	</Card>
	<Card title="Social" icon="setting">
		Coming Soon!
	</Card>
</CardGrid>
